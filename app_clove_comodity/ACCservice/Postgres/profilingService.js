...
//service lamaran buruh
  async checkLamaran(user, jenis_pekerjaan, upah_harapan_rp, indikator_ukur, catatan) {
    const query = {
      ...
      values: [user, jenis_pekerjaan, upah_harapan_rp, indikator_ukur, catatan],
    };
    const result = await this._pool.query(query);
    if (result.rows.length === 0) {  return ;}
    throw new InvariantError('sudah ada data lamaran dengan nilai yang sama.');
  }    
  async addLamaran( user, {jenis_pekerjaan, upah_harapan_rp, indikator_ukur, catatan}) {
    await this.checkLamaran(user, jenis_pekerjaan, upah_harapan_rp, indikator_ukur, catatan);
    const id =`lamaran-${nanoid(5)}`;      
    const query ={
      text :`INSERT INTO lamaran_kerja VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`,
      values:[ id, user, jenis_pekerjaan, upah_harapan_rp, indikator_ukur, catatan, 1],
    };
    const result = await this._pool.query(query);
    if(!result.rows.length){
  ...
  async getLamaran(user){
    const query = {
      text: `SELECT * FROM lamaran_kerja WHERE owner_user = $1;`,
      values: [user],
    };
    const result = await this._pool.query(query);
  ...
  async updateLamaran(user, id, {jenis_pekerjaan, upah_harapan_rp, indikator_ukur, catatan}) {
    await this.checkKontak(user, jenis_pekerjaan, upah_harapan_rp, indikator_ukur, catatan);
    const query ={
      text :`UPDATE lamaran_kerja SET jenis_pekerjaan=$1 AND upah_harapan_rp=$2 AND indikator_ukur=$3 AND catatan=$4 WHERE id = $5 RETURNING id;`,
      values:[jenis_pekerjaan, upah_harapan_rp, indikator_ukur, catatan, id,],
    };
    const result = await this._pool.query(query);
    if(!result.rows.length){
 ...
 //service lowongan kerja/job reqruiment acc
  ...
  async getLowongan(user){
    const query = {
      text: `SELECT * FROM lowongan_kerja WHERE owner_user = $1;`,
      values: [user],
    };
    const result = await this._pool.query(query);
  ...
  async updateLowongan(user, id, {jenis_pekerjaan, upah_rp, indikator_ukur, catatan}) {
    await this.checkLowongan(user, jenis_pekerjaan, upah_rp, indikator_ukur, catatan);
    const query ={
      text :`UPDATE lowongan_kerja SET jenis_pekerjaan=$1 AND upah_rp=$2 AND indikator_ukur=$3 AND catatan=$4 WHERE id = $5 RETURNING id;`,
...
//service penawaran komoditas
  async checkPenawaran(user, jenis_penawaran, jenis_komoditas, maksimal, minimal, satuan, harga_rp, catatan) {
    const query = {
      text: `SELECT id FROM penawaran_komoditas WHERE owner_user=$1 AND jenis_penawaran=$2 AND jenis_komoditas=$3 AND maksimal=$4 AND minimal=$5 AND satuan=$6 AND harga_rp=$7 AND catatan=$8;`,
      values: [user, jenis_penawaran, jenis_komoditas, maksimal, minimal, satuan, harga_rp, catatan],
    };
    const result = await this._pool.query(query);
    if (result.rows.length === 0) {  return ;}
    throw new InvariantError('sudah ada data penawaran dengan nilai yang sama.');
  }    
  async addPenawaran( user, {jenis_penawaran, jenis_komoditas, maksimal, minimal, satuan, harga_rp, catatan}) {
    ...
    const query ={
      ...
      values:[ id, user, jenis_penawaran, jenis_komoditas, maksimal, minimal, satuan, harga_rp, catatan, 1],
    };
    const result = await this._pool.query(query);
    if(!result.rows.length){
  ...
  async updatePenawaran(user, id, {jenis_penawaran, jenis_komoditas, maksimal, minimal, satuan, harga_rp, catatan}) {
    await this.checkKontak(user, jenis_pekerjaan, upah_harapan, indikator_ukur, catatan);
    const query ={
      text :`UPDATE penawaran_komoditas SET jenis_penawaran=$1 AND jenis_komoditas=$2 AND maksimal=$3 AND minimal=$4 AND satuan=$5 AND harga_rp=$6 AND catatan=$7 WHERE id = $8 RETURNING id;`,
      values:[jenis_penawaran, jenis_komoditas, maksimal, minimal, satuan, harga_rp, catatan, id],
    };
    const result = await this._pool.query(query);
    if(!result.rows.length){
...
//service penawaran komoditas
  async updateStatusLamaran(tabel, id, field, {status}) {
    const query ={
      text :`UPDATE ${tabel} SET ${field}=$1  WHERE id = $2 RETURNING id;`,
      values:[status, id],
    };
    const result = await this._pool.query(query);
    if(!result.rows.length){
