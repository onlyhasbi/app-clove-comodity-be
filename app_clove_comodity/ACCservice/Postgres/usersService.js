...
class usersService {
  ...
  async addUserBuruh({nomor_telpon, nama, sandi, tanggal_lahir, jenis_kelamin, alamat}) {
    const id_userForNomorTelpon = await this.checkNomorTelpon('buruh', nomor_telpon);
  ...
  async addUserAcc({jenis_pengguna, nomor_telpon, nama, sandi, alamat}) {
    const id_userForNomorTelpon = await this.checkNomorTelpon('pxp', nomor_telpon);
    if ( id_userForNomorTelpon !== "kosong" ){  throw new InvariantError('nomor_telpon sudah terdaftar, anda bisa login sebagi pengguna dengan Nomor telpon dan sandi yang sesuai');}
  ...
  async getUserBuruh(id){
    const query = {
      text: `SELECT id , nomor_telpon, nama, tanggal_lahir, jenis_kelamin, alamat FROM buruh WHERE id = $1;`,
      values: [id],
    };
    const result = await this._pool.query(query);
  ...
  async getUserAcc(id){
    const query = {
      text: `SELECT id , jenis_pengguna, nomor_telpon, nama, alamat FROM pxp WHERE id = $1;`,
      values: [id],
    };
    const result = await this._pool.query(query);
  ...
  async updateUserBuruh (id, {nama, tanggal_lahir, jenis_kelamin, alamat } ){
    const query = {
      text: `UPDATE buruh SET nama = $1, tanggal_lahir = $2, jenis_kelamin = $3, alamat = $4 WHERE id = $5 RETURNING id;`,
