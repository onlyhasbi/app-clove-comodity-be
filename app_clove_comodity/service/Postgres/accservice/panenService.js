...
//SERVICE SETORAN
    ...
    async checkSetoran(id_hasil_panen, id_buruh, waktu, catatan) {
      const result = await this._pool.query(`SELECT id FROM setoran WHERE id_hasil_panen = '${id_hasil_panen}' AND id_buruh = '${id_buruh}' AND waktu='${waktu}' AND catatan='${catatan}';`);
      if (result.rows.length === 0) {  return ;}
...
////SERVICE CRUD SETORAN
    async addSetoran({id_hasil_panen, id_buruh, volume_liter, berat_kg, upah_rp, waktu, catatan}) {
      await this.checkSetoran(id_hasil_panen, id_buruh, waktu, catatan);
    ...
    async getSetoran(user){
      const query = {
        text: `SELECT 
                ...
                setoran.kategori_konplaint,
                setoran.deskripsi_konplaint
              FROM 
                setoran 
